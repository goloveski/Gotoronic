...
const int speed_step = 8;					//8=9速 9=10速 10=11速
...
const int pos_shiftstep = 68;				// shift step degree 56->68 @160404 for 9speed
...
const int wait_msec1 =  500;				//多段変則設定時間1[msec]
const int wait_msec2 = 1000;				//多段変則設定時間2[msec]
const int wait_msec3 = 1500;				//多段変則設定時間3[msec]
...
const int pos_zero_f = 80;				// shift 0 position 180deg ->1st gear direction, 0deg ->10th gear direction, 
const int pos_inner = 70;					//80->70 @150809
const int pos_outer = 150;				//120->120 @150809
const int pos_shiftstep_f = 5;			// shift step degree
const int pos_shiftover_up_f = 40;		// shift over degree 15->20 @150809
const int pos_shiftover_down_f = 40;		// shift over degree 15->40 @150809
const int ovsft_delay_f = 2000;			// over shift delay [msec]
const int sft_delay_f = 20;				// shift delay per gear pos [msec]
...
{ 
	...
	for(n=2;n<= speed_step+1 ;n++)
	{
		pos_data[n] = EEPROM.read(pos_memory_addres + n -1 );

		if (pos_data[n] < 0)			//offset data 0より小を禁止
			pos_data[n] = 0;
		if (pos_data[n] > 200)			//offset data 200より大を禁止
			pos_data[n] = 0;
...
void loop()
{
	...
	boolean changed = 0,changed2 = 0,changed_f1 = 0,changed_f2 = 0,changed_SEL = 0;
...
//	 Serial.println(pos_final);
	...
	changed = debouncer.update();
	changed2 = debouncer2.update();
	changed_f1 = debouncer3.update();
	changed_f2 = debouncer4.update();
	changed_SEL = debouncerX.update();
...
//		delay(100); 
	...
	buf=atoi(input);//シリアル入力された文字列をint型に変換
